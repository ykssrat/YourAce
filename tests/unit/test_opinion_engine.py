...
def test_generate_opinion_matrix_returns_expected_keys() -> None:
    ...
    result = generate_opinion_matrix(close, long_fund_trend=0.02)

    assert set(result.keys()) == {"short", "mid", "long"}


def test_generate_opinion_matrix_labels_are_valid() -> None:
    """输出标签应在允许集合中。"""
    close = pd.Series(np.linspace(20, 10, 130))
    result = generate_opinion_matrix(close, long_fund_trend=-0.01)

    for label in result.values():
        assert label in {"BUY", "HOLD", "SELL"}


def test_generate_opinion_matrix_with_short_series_raises() -> None:
    """样本过短时应抛出异常。"""
    close = pd.Series([10.0, 10.2, 10.1])
    with pytest.raises(ValueError):
        generate_opinion_matrix(close)
