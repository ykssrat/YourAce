...
# 只关注新闻抓取的测试用例
def test_local_api_news_only() -> None:
    """仅验证 include_news=True 时能返回新闻内容。"""
    if not _server_smoke_supported():
        pytest.skip("当前测试解释器缺少 FastAPI/uvicorn 依赖，跳过本地 API 新闻测试")

    result = run_local_api_smoke(code="000001", include_news=True)

    assert result["code"] == "000001"
    # 只要求 latest_news 字段为非空数组
    assert result.get("news_enabled") is True
    assert isinstance(result.get("latest_news"), list)
...
def _print_result(result: dict[str, Any], port: int, include_news: bool) -> None:
    ...
    for idx, news in enumerate(latest_news, 1):
        print(f"  [{idx}] {news}")
    print("\n原始响应 JSON:")
    print(json.dumps({"latest_news": latest_news}, ensure_ascii=False, indent=2))
